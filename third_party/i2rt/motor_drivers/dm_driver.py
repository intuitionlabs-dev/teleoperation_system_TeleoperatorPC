...
class MotorType:
    ...
    # get motor constants from motor type
    @classmethod
...
class CanInterface:
    ...
    def _send_message_get_response(
        self, id: int, motor_id: int, data: List[int], max_retry: int = 10, expected_id: Optional[int] = None
    ) -> can.Message:
        ...
        for _ in range(max_retry):
            try:
                ...
                response = self._receive_message(motor_id, timeout=0.3)
...
class DMChainCanInterface(MotorChain):
    ...
    def _motor_on(self) -> None:
        motor_feedback = []
        # Extra settle/flush to reduce transient startup races
        for _ in range(12):
