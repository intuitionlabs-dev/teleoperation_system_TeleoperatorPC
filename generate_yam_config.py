...
# Add third_party to path for gello imports
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "third_party")))
...
class Args:
    ...
    """Output path for the generated YAML config. If not provided, will use third_party/configs/yam_auto_generated.yaml"""

    sim_output_path: Optional[str] = None
    """Output path for the simulation YAML config. If not provided, will use third_party/configs/yam_auto_generated_sim.yaml"""
    ...
    """CAN channel for YAM robot communication. If not provided, will auto-detect based on port."""
...
def detect_yam_channel(port: str) -> str:
    """Detect YAM channel based on the GELLO port.
    
    Args:
        port: The port that GELLO is connected to
        
    Returns:
        The appropriate CAN channel name
    """
    if "/dev/ttyACM0" in port:
        return "can_follow_r"  # Right arm
    elif "/dev/ttyACM1" in port:
        return "can_follow_l"  # Left arm
    else:
        # Default to left if we can't determine
        print(f"Warning: Could not determine arm side from port {port}, defaulting to left arm (can_follow_l)")
        return "can_follow_l"


def find_gello_port() -> Optional[str]:
    """Auto-detect GELLO port.

    Preference order (default to tty devices):
    1) /dev/ttyACM*
    2) /dev/ttyUSB*
    3) /dev/serial/by-id/usb-FTDI_*

    If multiple are found, prompt the user to select.
    Returns the selected device path or None if nothing found.
    """
    def _find_ports(patterns: list[str]) -> list[str]:
        ports: list[str] = []
    ...
    # 1) Prefer common Linux device names
    possible_ports = _find_ports(["/dev/ttyACM*", "/dev/ttyUSB*"])

    # 2) Fallback to FTDI by-id
    if not possible_ports:
        possible_ports = _find_ports(["/dev/serial/by-id/usb-FTDI_USB__-__Serial_Converter_*"])
...
def main(args: Args) -> None:
    ...
    # Step 1.5: Channel detection
    if args.channel is None:
        channel = detect_yam_channel(port)
        print(f"Detected YAM channel: {channel}")
    else:
    ...
    try:
        ...
        hardware_config = update_config_with_offsets(
            hardware_template, port, joint_offsets, gripper_config, channel
        )
        sim_config = update_config_with_offsets(
            sim_template, port, joint_offsets, gripper_config, channel
        )
